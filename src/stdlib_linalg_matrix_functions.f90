...
contains
    ...
    module subroutine stdlib_linalg_s_expm_inplace(A, order, err)
        ...
        if (m /= n) then
            ...
            do concurrent(i=1:n, j=1:n)
                A(i, j) = merge(1.0_sp + c*A(i, j), c*A(i, j), i == j)
                Q(i, j) = merge(1.0_sp - c*Q(i, j), -c*Q(i, j), i == j)
            enddo
    ...
    module subroutine stdlib_linalg_d_expm_inplace(A, order, err)
        ...
        if (m /= n) then
            ...
            allocate (Q, source=A2) ; A = A2
            do concurrent(i=1:n, j=1:n)
                A(i, j) = merge(1.0_dp + c*A(i, j), c*A(i, j), i == j)
                Q(i, j) = merge(1.0_dp - c*Q(i, j), -c*Q(i, j), i == j)
            enddo
